...
<xsl:stylesheet version="2.0" xmlns:gmd="http://www.isotc211.org/2005/gmd"
    ...
    <!-- Just contains the bfs fields related templates -->
  ...
  <xsl:template match="/">
      ...
      <xsl:if test="geonet:info/isTemplate != 's'">
            ...
            <xsl:apply-templates select="*[name(.)='bfs:MD_Metadata' or @gco:isoType='bfs:MD_Metadata']" mode="metadata"/>
    ...
    <xsl:template match="*" mode="metadata">
        ...
        <!-- Index thesaurus type -->
          ...
          <xsl:if test="$indexAllKeywordDetails">
            ...
            <xsl:variable name="email" select="/bfs:MD_Metadata/gmd:contact[1]/gmd:CI_ResponsibleParty[1]/gmd:contactInfo[1]/gmd:CI_Contact[1]/gmd:address[1]/gmd:CI_Address[1]/gmd:electronicMailAddress[1]/gco:CharacterString[1]"/>
        ...
        <xsl:for-each select="gmd:onLine/gmd:CI_OnlineResource[gmd:linkage/gmd:URL!='']">
          ...
          <xsl:if test="starts-with($protocol,'OGC:WMS') and
            ...
            <Field name="link" string="{concat($title, '|', $desc, '|',
                        '../../srv/en/google.kml?uuid=', /bfs:MD_Metadata/gmd:fileIdentifier/gco:CharacterString, '&amp;layers=', $title,
